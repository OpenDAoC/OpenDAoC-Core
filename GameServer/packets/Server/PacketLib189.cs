...
namespace DOL.GS.PacketHandler
{
	...
	public class PacketLib189 : PacketLib188
	{
		...
		protected virtual void WriteItemData(GSTCPPacketOut pak, DbInventoryItem item)
		{
            ...
            if (item.SellPrice > 0)
            {
				if (ServerProperties.Properties.CONSIGNMENT_USE_BP)
                    ...
                    name += "[" + WalletHelper.ToShortString(item.SellPrice) + "]";
            }
		...
		{
			using (GSTCPPacketOut pak = GSTCPPacketOut.GetForTick(p => p.Init(GetPacketCode(eServerPackets.HouseCreate))))
			{
		...
		{
			using (GSTCPPacketOut pak = GSTCPPacketOut.GetForTick(p => p.Init(GetPacketCode(eServerPackets.HouseChangeGarden))))
			{
				pak.WriteShort((ushort)house.HouseNumber);
		...
		{
			using (GSTCPPacketOut pak = GSTCPPacketOut.GetForTick(p => p.Init(GetPacketCode(eServerPackets.HouseEnter))))
			{
				pak.WriteShort((ushort)house.HouseNumber);
