					...
					if (client.Account.PrivLevel == (int)ePrivLevel.Player)
					{
						...
						if (keep != null)
						{
							...
							// this handles only keeps outside our own realm
                                                        ...
                                                        if (client.Player.Realm != keep.OriginalRealm && ((supply1.Realm != supply1.OriginalRealm || (supply1 as GameKeep).OwnsAllTowers == false) ||
                                                                (supply2.Realm != supply2.OriginalRealm || (supply2 as GameKeep).OwnsAllTowers == false)))
                                                        {
                                                            client.Player.Out.SendMessage("The supply line to the keep is broken.", eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                                            return;
                                                        }
