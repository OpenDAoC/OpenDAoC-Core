            ...
            if (allChars != null)
            {
                foreach (DbCoreCharacters character in allChars.ToArray())
                {
                    if (character.AccountSlot == charSlot && client.ClientState == GameClient.eClientState.CharScreen)
                    {
						...
						{
							var backupCharacter = new DbCoreCharacterBackups(character);
                        ...
                        else
                        {
                            ...
                            try
                            {
                                var objs = GameServer.Database.SelectObjects<DbInventoryItems>(DB.Column("OwnerID").IsEqualTo(character.ObjectId));
