	...
	public class HousingPlaceItemHandler : IPacketHandler
	{
		...
		public void HandlePacket(GameClient client, GSPacketIn packet)
		{
			try
			{
				...
				DbInventoryItems orgitem = client.Player.Inventory.GetItem((eInventorySlot)slot);
				if (orgitem == null)
				{
					client.Player.Out.SendInventorySlotsUpdate(null);
					return;
		...
		private static bool IsSuitableForWall(DbInventoryItems item)
		{
		...
		private static bool IsSuitableForHookpoint(DbInventoryItems item)
		{
			switch (item.Object_Type)
			{
		...
		private void LogLocation(GamePlayer player, byte response)
		{
			...
			var a = new DbHouseHookPointOffsets
			{
