...
namespace Core.GS.Commands
{
	...
	public class PlvlCommand : ACommandHandler, ICommandHandler
	{
				...
				case "single":
				{
					switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							SingleCommandPermission.setPermission(target, args[2]);
				...
				case "singleaccount":
				{
					switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							SingleCommandPermission.setPermissionAccount(target, args[2]);
				...
				case "remove":
				{
					switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							if (SingleCommandPermission.HasPermission(target, args[2]))
							{
								// Removes the command type to the 'singlepermission' table
								SingleCommandPermission.removePermission(target, args[2]);
							}
                ...
                case "removeaccount":
                {
	                switch (args.Length)
					{
						...
						case >= 4:
						{
							...
							if (SingleCommandPermission.HasPermission(target, args[2]))
							{
								SingleCommandPermission.removePermissionAccount(target, args[2]);
							}
				...
				default:
					{
						...
						if (args[1] == "1" || args[1] == "2" && client.Player == target && target == null)
						{
							...
							if (SingleCommandPermission.HasPermission(client.Player, "plvl") == false)
							{
