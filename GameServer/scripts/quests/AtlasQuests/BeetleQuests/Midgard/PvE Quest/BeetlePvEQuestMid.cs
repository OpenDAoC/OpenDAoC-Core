...
namespace DOL.GS.BaseQuest.Midgard
{
	public abstract class BeetlePvEQuestMid : Quests.BaseQuest
	{
		...
		public override void Notify(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (gArgs.Target.Name.ToLower() == middragon.Name.ToLower() && gArgs.Target is GameNPC && _dragonKilled < MAX_KILLED)
			{
				_dragonKilled = 1;
				player.Out.SendMessage("[Beetle] You killed " + middragon.Name + ": (" + _dragonKilled + " | " + MAX_KILLED + ")", eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
