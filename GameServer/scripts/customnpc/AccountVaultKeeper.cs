...
namespace DOL.GS
{
    ...
    public class AccountVault : GameHouseVault
    {
        ...
        public override Dictionary<int, DbInventoryItems> GetClientInventory(GamePlayer player)
        {

            var items = new Dictionary<int, DbInventoryItems>();
        ...
        public override bool MoveItem(GamePlayer player, ushort fromSlot, ushort toSlot)
        {
            ...
            if (toAccountVault)
            {
                DbInventoryItems item = player.Inventory.GetItem((eInventorySlot)toSlot);
                if (item != null)
                {
        ...
        /// </summary>
        private new IList<DbInventoryItems> DBItems(GamePlayer player = null)
        {
            return GameServer.Database.SelectObjects<DbInventoryItems>(DB.Column("OwnerID").IsEqualTo(GetOwner(player)).And(DB.Column("SlotPosition").IsGreaterOrEqualTo(FirstDBSlot).And(DB.Column("SlotPosition").IsLessOrEqualTo(LastDBSlot))));
        }
