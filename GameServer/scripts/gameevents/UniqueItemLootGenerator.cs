    ...
    public class LootGeneratorUniqueItem : LootGeneratorBase
    {
            ...
            DOL.GS.Commands.ICommandHandler
        {
            public void OnCommand(GameClient client, string[] args)
            {
                ...
                {
                    foreach (DbInventoryItems item in client.Player.Inventory.GetItemRange(eInventorySlot.FirstBackpack,
         ...
         private static void GenerateWeapon(GameLiving player, eCharacterClass charClass, eObjectType type, eInventorySlot invSlot)
        {
			...
			else if (type == eObjectType.Flexible)
            {
				...
				DbInventoryItems tempItem = GameInventoryItem.Create<DbItemUniques>(dmgTypeItem);
				...
				DbInventoryItems tempItem2 = GameInventoryItem.Create<DbItemUniques>(dmgTypeItem2);
