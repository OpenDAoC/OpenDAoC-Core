...
namespace DOL.GS.Friends
{
	...
	public sealed class FriendsManager
	{
		...
		public void AddPlayerFriendsListToCache(GamePlayer Player)
		{
			...
			if (friends.Any())
				offlineFriends = Database.SelectObjects<DbCoreCharacters>(DB.Column("Name").IsIn(friends)).Select(chr => new FriendStatus(chr.Name, chr.Level, chr.Class, chr.LastPlayed)).ToArray();
		...
		public bool AddFriendToPlayerList(GamePlayer Player, string Friend)
		{
			...
			DbCoreCharacters offlineFriend = Database.SelectObjects<DbCoreCharacters>(DB.Column("Name").IsEqualTo(Friend)).FirstOrDefault();
