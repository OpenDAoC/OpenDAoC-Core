    ...
    public class OfRaVolleyEcsEffect : EcsGameAbilityEffect
    {
        ...
        public void DecideNextShoot()
        {
            ...
            if (_remainingShots == 0)
            {
                ...
                Cancel(false);
                OfRaVolleyAbility volley = OwnerPlayer.GetAbility<OfRaVolleyAbility>();
                OwnerPlayer.DisableSkill(volley, OfRaVolleyAbility.DISABLE_DURATION);
            }
        }
        ...
        private void PlayerUseVolley(CoreEvent e, object sender, EventArgs args)
        {
            ...
            if (player.IsWithinRadius(player.GroundTarget, OfRaVolleyAbility.GetMinAttackRange(player.Realm)))
            {
                player.Out.SendMessage("You ground target is too close to use Volley!", EChatType.CT_System, EChatLoc.CL_SystemWindow);
                return;
            }

            if (!player.IsWithinRadius(player.GroundTarget, OfRaVolleyAbility.GetMaxAttackRange(player.Realm)))
            {
