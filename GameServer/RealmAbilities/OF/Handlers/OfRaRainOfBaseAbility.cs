...
public abstract class OfRaRainOfBaseAbility : TimedRealmAbility
{
    ...
    public OfRaRainOfBaseAbility(DbAbility ability, int level) : base(ability, level) { }
    ...
    protected virtual void Execute(string name, int icon, int clientEffect, int damageType, GameLiving living)
    {
        ...
        new OfRaRainOfBaseEcsEffect(new EcsGameEffectInitParams(living, duration, 1, spellHandler));
        DisableSkill(living);
    }
