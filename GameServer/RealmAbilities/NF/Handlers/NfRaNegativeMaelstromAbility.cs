...
public class NfRaNegativeMaelstromAbility : TimedRealmAbility
{
    public NfRaNegativeMaelstromAbility(DbAbility dba, int level) : base(dba, level) { }
	...
	protected virtual int EndCast(EcsGameTimer timer)
	{
        ...
        if (!castWasSuccess)
            return 0;
		NegativeMaelstromBase nm = new NegativeMaelstromBase(dmgValue);
