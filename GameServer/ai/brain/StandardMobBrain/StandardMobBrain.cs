...
public class StandardMobBrain : APlayerVicinityBrain, IOldAggressiveBrain
{
    ...
    public const int
        MAX_PET_AGGRO_DISTANCE = 512; // Tolakram - Live test with caby pet - I was extremely close before auto aggro
    ...
    {
        return base.ToString() + ", m_aggroLevel=" + m_aggroLevel + ", m_aggroMaxRange=" + m_aggroMaxRange;
    }
    ...
    {
        var range = (ushort) (ThinkInterval / 800 * Body.CurrentWayPoint.MaxSpeed);
    ...
    protected GameObject m_targetLOSObject;
    ...
    public virtual bool HasAggressionTable()
    {
        if (AggroRange > 0)
        {
            ...
            for (var i = 0; i < PlayersSeen.Count; i++)
    ...
    public virtual bool IsBeyondTetherRange()
    {
        ...
        {
            var distance = Body.GetDistanceTo(Body.SpawnPoint);
            var maxdistance = Body.MaxDistance > 0 ? Body.MaxDistance : -Body.MaxDistance * AggroRange / 100;
            if (maxdistance > 0 && distance > maxdistance)
                return true;
            return false;
        }
    ...
    public virtual void CheckNPCAggro()
    {
                     ...
                     Body.CurrentRegion.IsDungeon ? false : true))
        {
            ...
            if (npc.Brain is IControlledBrain && (npc.Brain as IControlledBrain).GetPlayerOwner() != null)
            {
                ...
                if (!Util.Chance(aggrolevel)) return;
            }
    ...
    public GamePlayer LosChecker(GameLiving actionSource, GameObject actionTarget)
    {
        if (actionSource == null || actionTarget == null)
            return null;

        if (actionSource is GamePlayer) return actionSource as GamePlayer;
        ...
        if (actionSource is GameNPC &&
            ...
            ((actionSource as GameNPC).Brain as IControlledBrain).GetPlayerOwner().ObjectState ==
            GameObject.eObjectState.Active)
        ...
        if (actionTarget is GameNPC &&
            ...
            ((actionTarget as GameNPC).Brain as IControlledBrain).GetPlayerOwner().ObjectState ==
            GameObject.eObjectState.Active)
        ...
        foreach (GamePlayer pl in actionSource.GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE))
            if (pl != null && pl.ObjectState == GameObject.eObjectState.Active)
                return pl;
        return null;
    }

    protected void CheckLOS(GamePlayer CheckLOSPlayer, GameObject actionTarget)
    {
    ...
    protected void TempCheckLOS(GamePlayer CheckLOSPlayer, GameLiving Cible, int AggroAmount, bool natural)
    {
        if (CheckLOSPlayer == null || Cible == null || AggroAmount == 0)
            return;

        CheckLOSPlayer.TempProperties.setProperty(
            "CHECKLOS_" + (ushort) Body.ObjectID + "_" + (ushort) Cible.ObjectID + "_living", Cible);
        CheckLOSPlayer.TempProperties.setProperty(
            "CHECKLOS_" + (ushort) Body.ObjectID + "_" + (ushort) Cible.ObjectID + "_aggroamount", AggroAmount);
        CheckLOSPlayer.TempProperties.setProperty(
            "CHECKLOS_" + (ushort) Body.ObjectID + "_" + (ushort) Cible.ObjectID + "_natural", natural);
    }

    protected void BeforeAddToAggro_CheckLOS(GamePlayer player, ushort response, ushort targetOID)
    {
        var living =
            player.TempProperties.getProperty<GameLiving>(
                "CHECKLOS_" + (ushort) Body.ObjectID + "_" + targetOID + "_living", null);
        var aggroamount =
            player.TempProperties.getProperty("CHECKLOS_" + (ushort) Body.ObjectID + "_" + targetOID + "_aggroamount",
                0);
        var natural =
            player.TempProperties.getProperty("CHECKLOS_" + (ushort) Body.ObjectID + "_" + targetOID + "_natural",
                true);
        ...
        else
        {
            if (living != null)
                ...
                {
                    if (GetAggroAmountForLiving(living) > 0) RemoveFromAggroList(living);
                    Body.TargetObject = null;
    ...
    public virtual void CheckPlayerAggro(bool useLOS)
    {
        ...
        if (!CheckForProximityAggro || Body == null || Body.CurrentZone == null) return;
        ...
        foreach (GamePlayer player in Body.GetPlayersInRadius((ushort) AggroRange, false))
        {
            ...
            if (aggroleveltotarget > 51)
            {
                if (Properties.ALWAYS_CHECK_LOS)
                {
                    ...
                    var losChecker = LosChecker(Body, player);
                    if (losChecker != null)
                    {
    ...
    /// </summary>
    public Dictionary<GameLiving, long> AggroTable => m_aggroTable;
    ...
    public void AddAggroListTo(StandardMobBrain brain)
    {
        ...
        lock ((m_aggroTable as ICollection).SyncRoot)
        {
            var dictEnum = m_aggroTable.GetEnumerator();
    ...
    public virtual bool AggroLOS { get; set; }
    ...
    public virtual void AddToAggroList(GameLiving living, int aggroamount, bool CheckLOS)
    {
        ...
        if (m_aggroTable.Count < 1) Body.FireAmbientSentence(GameNPC.eAmbientTrigger.aggroing, living);
        ...
        if (living is GamePlayer && aggroamount > 0)
        {
                         ...
                         .Where(e => e.EffectType == eEffect.Protect))
            {
                ...
                var abilityLevel = protect.ProtectSource.GetAbilityLevel(Abilities.Protect);
                var protectAmount = (int) (abilityLevel * 0.10 * aggroamount);

                if (protectAmount > 0)
                {
                    aggroamount -= protectAmount;
                    protect.ProtectSource.Out.SendMessage(LanguageMgr.GetTranslation(
                            protect.ProtectSource.Client.Account.Language, "AI.Brain.StandardMobBrain.YouProtDist",
                            player.GetName(0, false),
        ...
        lock ((m_aggroTable as ICollection).SyncRoot)
        {
            ...
            {
                var amount = m_aggroTable[living];
                amount += aggroamount;
    ...
    public void PrintAggroTable()
    {
        ...
        foreach (var gl in m_aggroTable.Keys) sb.AppendLine("Living: " + gl.Name + ", aggro: " + m_aggroTable[gl]);
    ...
    public virtual long GetAggroAmountForLiving(GameLiving living)
    {
        lock ((m_aggroTable as ICollection).SyncRoot)
        {
            if (m_aggroTable.ContainsKey(living)) return m_aggroTable[living];
            return 0;
        }
    }
    ...
    public virtual void AttackMostWanted()
    {
        ...
        if (Diagnostics.AggroDebugEnabled) PrintAggroTable();
    ...
    protected virtual GameLiving CalculateNextAttackTarget()
    {
        ...
        {
            double maxAggro = 0;
            var aggros = m_aggroTable.GetEnumerator();
            var removable = new List<GameLiving>();
            while (aggros.MoveNext())
            {
                var living = aggros.Current.Key;
                if (living == null)
                    continue;
                ...
                var amount = aggros.Current.Value;
                ...
                if (living.IsAlive
                    ...
                    && living.ObjectState == GameObject.eObjectState.Active)
                {
                    var distance = Body.GetDistanceTo(living);
                    // int maxAggroDistance = (this is IControlledBrain) ? MAX_PET_AGGRO_DISTANCE : MAX_AGGRO_DISTANCE;
                    var maxAggroDistance = MAX_AGGRO_DISTANCE;
                    if (distance <= maxAggroDistance)
                    {
                        var aggro = amount * Math.Min(500.0 / distance, 1);
                        if (aggro > maxAggro)
                        {
            ...
            if (maxAggroObject == null) m_aggroTable.Clear();
        }
    ...
    public virtual int CalculateAggroLevelToTarget(GameLiving target)
    {
        ...
        {
            var brain = ((GameNPC) target).Brain as IControlledBrain;
            if (brain != null)
            {
                var thisLiving = (((GameNPC) target).Brain as IControlledBrain).GetLivingOwner();
                if (thisLiving != null)
        ...
        if (Body.Faction != null)
        {
            ...
            {
                var player = (GamePlayer) target;
                AggroLevel = Body.Faction.GetAggroToFaction(player);
            }
            else if (target is GameNPC)
            {
                var npc = (GameNPC) target;
    ...
    public virtual void OnAttackedByEnemy(AttackData ad)
    {
            ...
            && Body.ObjectState == GameObject.eObjectState.Active)
        {
            ...
            {
                if (this is CommanderBrain cBrain) cBrain.Attack(ad.Attacker);
    ...
    protected virtual void BringFriends(GameLiving attacker)
    {
        ...
        var bg = puller.TempProperties.getProperty<object>(BattleGroup.BATTLEGROUP_PROPERTY, null) as BattleGroup;
        ...
        if (puller.Group is Group group)
        {
            ...
            foreach (var player in group.GetPlayersInTheGroup())
                if (player != null && (player.InternalID == puller.InternalID ||
                                       player.IsWithinRadius(puller, BAFPlayerRange, true)))
                {
                    numAttackers++;
                           ...
                           || (Properties.BAF_MOBS_ATTACK_BG_MEMBERS && !Properties.BAF_MOBS_ATTACK_PULLER)))
        {
            ...
            foreach (var player in bg.GetPlayersInTheBattleGroup())
                if (player != null && (player.InternalID == puller.InternalID ||
                                       player.IsWithinRadius(puller, BAFPlayerRange, true)))
                {
        ...
        var percentBAF = Properties.BAF_INITIAL_CHANCE
                         + (numAttackers - 1) * Properties.BAF_ADDITIONAL_CHANCE;

        var maxAdds = percentBAF / 100; // Multiple of 100 are guaranteed BAFs
        ...
        if (maxAdds > 0)
        {
            var numAdds = 0; // Number of mobs currently BAFed
            var range = BAFInitialRange; // How far away to look for friends
            ...
            if (type == eCheckSpellType.Defensive)
            {
                foreach (Spell spell in Body.Spells)
                {
                    ...
                    if (Body.ControlledBrain == null)
                        if (spell.SpellType == (byte) eSpellType.Pet)
                            continue;
                    ...
                    {
                        if (Util.Chance(30) && Body.ControlledBrain != null &&
                            spell.SpellType == (byte) eSpellType.Heal &&
        ...
        switch (spell.SpellType)
        {
            ...
            case (byte) eSpellType.DamageShield:
            {
                ...
                if (Body.ControlledBrain != null && Body.ControlledBrain.Body != null && Util.Chance(40) &&
                    ...
                    spell.Target.ToLower() != "self") Body.TargetObject = Body.ControlledBrain.Body;
                break;
            }
            ...
            case (byte) eSpellType.SpreadHeal:
                if (spell.Target.ToLower() == "self")
                {
                    ...
                    if (Body.HealthPercent < Properties.NPC_HEAL_THRESHOLD) Body.TargetObject = Body;
                    break;
                }
    ...
    protected virtual bool CheckInstantSpells(Spell spell)
    {
        ...
        switch (spell.SpellType)
        {
            ...
            case (byte) eSpellType.Taunt:
                if (!LivingHasEffect(lastTarget as GameLiving, spell)) Body.TargetObject = lastTarget;
                break;
            ...
            case (byte) eSpellType.OffensiveProc:
                if (!LivingHasEffect(Body, spell)) Body.TargetObject = Body;
                break;
    ...
    protected bool LivingIsPoisoned(GameLiving target)
    {
        foreach (var effect in target.EffectList)
        {
            ...
            var speffect = effect as GameSpellEffect;
    ...
    {
        var maxRoamingRadius = Body.CurrentRegion.IsDungeon ? 5 : 500;
