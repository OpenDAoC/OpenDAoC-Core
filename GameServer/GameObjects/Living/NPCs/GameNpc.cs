...
namespace Core.GS
{
	...
	public class GameNpc : GameLiving, ITranslatableObject
	{
		...
		private EPetFormationType m_formation = EPetFormationType.Line;
		/// <summary>
		/// How the minions line up with the commander
		/// </summary>
		public EPetFormationType Formation
		{
		...
		/// </summary>
		public virtual ENpcFlags Flags
		{
			get { return m_flags; }
			set
			{
				ENpcFlags oldflags = m_flags;
				m_flags = value;
				if (ObjectState == eObjectState.Active)
				{
					if (oldflags != m_flags)
		...
		{
			get { return (m_flags & ENpcFlags.SWIMMING) == ENpcFlags.SWIMMING || base.IsUnderwater; }
		}
		...
		public virtual void LoadTemplate(INpcTemplate template)
		{
			...
			Flags = (ENpcFlags)template.Flags;
		...
		public override bool AddToWorld()
		{
			...
			if (GameServer.Instance.ServerStatus == EGameServerStatus.GSS_Open)
				FireAmbientSentence(EAmbientNpcTrigger.spawning, this);
			...
			if (ShowTeleporterIndicator)
			{
				if (m_teleporterIndicator == null)
				{
					m_teleporterIndicator = new GameNpc
					{
						...
						Flags = ENpcFlags.PEACE | ENpcFlags.CANTTARGET | ENpcFlags.DONTSHOWNAME | ENpcFlags.FLYING
					};
				}
			...
			if (Flags.HasFlag(ENpcFlags.STEALTH))
				WasStealthed = true;
		...
		public override bool Interact(GamePlayer player)
		{
			...
			FireAmbientSentence(EAmbientNpcTrigger.interact, player);
			return true;
		}
		...
		public override void ProcessDeath(GameObject killer)
		{
			...
			try
			{
				...
				FireAmbientSentence(EAmbientNpcTrigger.dying, killer);
		...
		public override void EnemyHealed(GameLiving enemy, GameObject healSource, EHealthChangeType changeType, int healAmount)
		{
			...
			if (Brain is StandardMobBrain mobBrain)
			{
				...
				{
					if (healSource is GamePlayer || (healSource is GameNpc && (((GameNpc)healSource).Flags & ENpcFlags.PEACE) == 0))
					{
		...
		public virtual void FireAmbientSentence(EAmbientNpcTrigger trigger, GameObject living)
		{
			if (IsSilent || ambientTexts == null || ambientTexts.Count == 0) return;
			if (trigger == EAmbientNpcTrigger.interact && living == null) return; // Do not trigger interact messages with a corpse
