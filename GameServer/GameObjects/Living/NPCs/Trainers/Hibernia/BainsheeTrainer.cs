...
public class BainsheeTrainer : GameTrainer
{
	...
	{
		get { return EPlayerClass.Bainshee; }
	}
	...
	public override bool WhisperReceive(GameLiving source, string text)
	{
		...
		if (lowerCase == LanguageMgr.GetTranslation(player.Client.Account.Language, "BainsheeTrainer.WhisperReceiveCase.Text1"))
		{
			...
			if (CanPromotePlayer(player))
			{
				PromotePlayer(player, (int)EPlayerClass.Bainshee, LanguageMgr.GetTranslation(player.Client.Account.Language, "BainsheeTrainer.WhisperReceive.Text1", player.GetName(0, false)), null);
