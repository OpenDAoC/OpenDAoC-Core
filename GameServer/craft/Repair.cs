...
namespace DOL.GS
{
	...
	public class Repair
	{
		...
		protected static int Proceed(ECSGameTimer timer)
		{
			...
			DbInventoryItems item = timer.Properties.GetProperty<DbInventoryItems>(AbstractCraftingSkill.RECIPE_BEING_CRAFTED, null);
		...
		/// <returns></returns>
		public static bool IsAllowedToBeginWork(GamePlayer player, DbInventoryItems item, int percentNeeded)
		{
		...
		/// </summary>
		protected static int GetRepairTime(GamePlayer player, DbInventoryItems item)
		{
		...
		/// </summary>
		protected static int CalculateSuccessChances(GamePlayer player, DbInventoryItems item)
		{
			eCraftingSkill skill = CraftingMgr.GetSecondaryCraftingSkillToWorkOnItem(item);
