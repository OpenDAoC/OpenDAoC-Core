...
public class PvpServerRules : AServerRules
{
	...
	public override void OnReleased(CoreEvent e, object sender, EventArgs args)
	{
		...
		if (player.TempProperties.GetProperty<string>(KILLED_BY_PLAYER_PROP, null) != null)
		{
			...
			StartImmunityTimer(player, ServerProperty.TIMER_KILLED_BY_PLAYER * 1000);//When Killed by a Player
		}
		else
		{
	...
	public override void OnPlayerTeleport(GamePlayer player, GameLocation source, DbTeleport destination)
	{
		...
		{
			StartImmunityTimer(player, ServerProperty.TIMER_PVP_TELEPORT * 1000);
		}
	}
