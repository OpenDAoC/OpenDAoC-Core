...
public class ServerListUpdate
{
	...
	{
        if(ServerProperty.SERVERLISTUPDATE_ENABLED)
		    Init();
	}
	...
	private static void StartList()
	{
		...
		string Updater = UrlEncode($"{UpdateURL}&username={ServerProperty.SERVER_LIST_UPDATE_USER}&password={ServerProperty.SERVER_LIST_UPDATE_PASS}&totalclients={ClientService.ClientCount}&version={ScriptVersion}");
	...
	public static string[] ValidateArgs()
	{
		...
		int count = 0;
        if (string.IsNullOrEmpty(ServerProperty.SERVER_LIST_UPDATE_USER))
		{
			failed[count] = "Username";
			count++;
		}
		if (string.IsNullOrEmpty(ServerProperty.SERVER_LIST_UPDATE_PASS))
		{
