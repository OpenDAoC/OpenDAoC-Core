    ...
    public class DayNightCycleTimer(GameObject owner) : ECSGameTimerWrapperBase(owner)
    {
        ...
        private void InitializeInternal(uint dayIncrement, uint dayStart)
        {
            lock (_lock)
            {
                ...
                if (_updateInterval < GameLoop.TICK_RATE && log.IsWarnEnabled)
                {
                    log.Warn($"The update interval ({_updateInterval}) had to be set to a value lower than the server tick rate ({GameLoop.TICK_RATE}) when trying to ensure two updates happen per in-game minute. " +
