...
namespace DOL.GS.Keeps
{
	...
	public class GameKeepHookPoint : Point3D
	{
		...
		private void ObjectDie(DOLEvent e, object sender, EventArgs arguments)
		{
			...
			var item = DOLDB<DbKeepHookPointItems>.SelectObject(DB.Column("KeepID").IsEqualTo(Component.Keep.KeepID).And(DB.Column("ComponentID").IsEqualTo(Component.ID)).And(DB.Column("HookPointID").IsEqualTo(ID)));
			if (item != null)
