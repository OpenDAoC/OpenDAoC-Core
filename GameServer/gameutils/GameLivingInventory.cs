...
namespace DOL.GS
{
	...
	public abstract class GameLivingInventory : IGameInventory
	{
		...
		/// </summary>
		protected readonly Dictionary<eInventorySlot, DbInventoryItems> m_items;
		...
		{
			m_items = new Dictionary<eInventorySlot, DbInventoryItems>();
		...
		public virtual ICollection<DbInventoryItems> GetItemRange(eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			...
			var items = new List<DbInventoryItems>();
		...
		public DbInventoryItems GetFirstItemByID(string uniqueID, eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			minSlot = GetValidInventorySlot(minSlot);
			maxSlot = GetValidInventorySlot(maxSlot);
			if (minSlot == eInventorySlot.Invalid || maxSlot == eInventorySlot.Invalid)
		...
		/// <param name="maxSlot">last slot for search</param>
		/// <returns>found item or null</returns>
		public DbInventoryItems GetFirstItemByObjectType(int objectType, eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			minSlot = GetValidInventorySlot(minSlot);
			maxSlot = GetValidInventorySlot(maxSlot);
			if (minSlot == eInventorySlot.Invalid || maxSlot == eInventorySlot.Invalid)
		...
		/// <param name="maxSlot">last slot for search</param>
		/// <returns>found item or null</returns>
		public DbInventoryItems GetFirstItemByName(string name, eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			minSlot = GetValidInventorySlot(minSlot);
			maxSlot = GetValidInventorySlot(maxSlot);
			if (minSlot == eInventorySlot.Invalid || maxSlot == eInventorySlot.Invalid)
		...
		public virtual bool AddItem(eInventorySlot slot, DbInventoryItems item)
		{
			if (item == null)
				return false;
		...
		public virtual bool AddTradeItem(eInventorySlot slot, DbInventoryItems item)
		{
			return false;
		}
		...
		{
			var tempList = new List<DbInventoryItems>(m_items.Count);
			foreach (var entry in m_items)
			{
				tempList.Add(entry.Value);
			}

			foreach (DbInventoryItems item in tempList)
			{
				RemoveItem(item);
			}
		}
		...
		public virtual bool RemoveItem(DbInventoryItems item)
		{
			lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
			{
				if (item == null)
		...
		public virtual bool RemoveTradeItem(DbInventoryItems item)
		{
			return false;
		}
		...
		/// <param name="count"></param>
		/// <returns></returns>
		public virtual bool AddCountToStack(DbInventoryItems item, int count)
		{
			if (item == null)
				return false;
		...
		public virtual bool RemoveCountFromStack(DbInventoryItems item, int count)
		{
			if (item == null)
				return false;
		...
		public virtual DbInventoryItems GetItem(eInventorySlot slot)
		{
			slot = GetValidInventorySlot(slot);
		...
		/// </summary>
		public virtual ICollection<DbInventoryItems> VisibleItems
		{
			get
			{
				var items = new List<DbInventoryItems>(VISIBLE_SLOTS.Length);
		...
		/// </summary>
		public virtual ICollection<DbInventoryItems> EquippedItems
		{
			get
			{
				var items = new List<DbInventoryItems>(EQUIP_SLOTS.Length);
		...
		/// </summary>
		public virtual ICollection<DbInventoryItems> AllItems
		{
		...
		public virtual bool AddTemplate(DbInventoryItems sourceItem, int count, eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			...
			lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
			{
				...
				try
				{
					foreach (var slot in changedSlots)
					{
						...
						{
							if (sourceItem.Template is DbItemUniques)
							{
		...
		public virtual bool RemoveTemplate(string templateID, int count, eInventorySlot minSlot, eInventorySlot maxSlot)
		{
			...
			lock (m_items) // Mannen 10:56 PM 10/30/2006 - Fixing every lock(this)
			{
				var changedSlots = new Dictionary<DbInventoryItems, int>();
		...
		protected virtual bool CombineItems(DbInventoryItems fromItem, DbInventoryItems toItem)
		{
			return false;
		}
		...
		{
			DbInventoryItems newFromItem;
			DbInventoryItems newToItem;
