...
namespace DOL.GS
{
    public class MarketSearch
    {
        ...
        public virtual List<DbInventoryItem> FindItems(SearchData search)
        {
            m_searchHasError = false;

            if (ServerProperties.Properties.MARKET_ENABLE_LOG && search.page == 0 && log.IsDebugEnabled)
                log.Debug($"Market Search: [{m_searchPlayer.Name}:{m_searchPlayer.Client.Account.Name}] SEARCHING MARKET: {search}");

            Stopwatch watch = Stopwatch.StartNew();

            IEnumerable<DbInventoryItem> candidates = GetInitialCandidates(search);
            candidates = ApplyFilters(candidates, search);
            List<DbInventoryItem> items = candidates.Take(ServerProperties.Properties.MARKET_SEARCH_LIMIT).ToList();

            watch.Stop();

            if (log.IsInfoEnabled)
                log.Info($"{nameof(FindItems)} took {watch.ElapsedMilliseconds}ms on {MarketCache.ItemCount} items, got {items.Count} results! searchText: {search.name}");

            if (ServerProperties.Properties.MARKET_ENABLE_LOG && log.IsDebugEnabled)
                log.Debug($"Returning '{items.Count}' items from search.");

            return items;
        }

        private static IEnumerable<DbInventoryItem> GetInitialCandidates(SearchData search)
        {
            ItemQuery query = new()
            {
        ...
        /// <summary>
        /// Apply all filters to candidates using efficient LINQ operations
        /// </summary>
        private IEnumerable<DbInventoryItem> ApplyFilters(IEnumerable<DbInventoryItem> candidates, SearchData search)
        {
            ...
            // Level filters.
            if (search.levelMin > 1)
                candidates = candidates.Where(item => item.Level >= search.levelMin);

            if (search.levelMax > -1 && search.levelMax < 52)
                candidates = candidates.Where(item => item.Level <= search.levelMax);

            // Quality filter.
            if (search.minQual > 84)
                candidates = candidates.Where(item => item.Quality >= search.minQual);

            // Price filters.
            if (search.priceMin > 1)
                candidates = candidates.Where(item => item.SellPrice >= search.priceMin);

            if (search.priceMax > 0)
                candidates = candidates.Where(item => item.SellPrice <= search.priceMax);

            // Armor type filter.
            if (search.armorType > 0)
                candidates = candidates.Where(item => CheckForArmorType(item, search.armorType));

            // Damage type filter.
            if (search.damageType > 0)
                candidates = candidates.Where(item => CheckForDamageType(item, search.damageType));

            // Bonus filters.
            if (search.bonus1 > 0)
                candidates = ApplyBonusFilter(candidates, search.bonus1, search.bonus1Value);

            if (search.bonus2 > 0)
                candidates = ApplyBonusFilter(candidates, search.bonus2, search.bonus2Value);

            if (search.bonus3 > 0)
                candidates = ApplyBonusFilter(candidates, search.bonus3, search.bonus3Value);

            return candidates;
        }

        /// <summary>
        /// Apply bonus filter based on bonus type
        /// </summary>
        private IEnumerable<DbInventoryItem> ApplyBonusFilter(IEnumerable<DbInventoryItem> items, int bonusType, int bonusValue)
        {
            switch (bonusType)
            {
                case 1: // Stats.
                    return items.Where(item => CheckForBonus(item, bonusValue));
                case 2: // Skills.
                    return items.Where(item => CheckForSkill(item, bonusValue));
                case 3: // Power.
                    return items.Where(item => CheckForPower(item, bonusValue));
                case 4: // HP.
                    return items.Where(item => CheckForHP(item, bonusValue));
                case 5: // Resists.
                    return items.Where(item => CheckForResist(item, bonusValue));
                default:
                    return [];
            }
        }
            ...
            if (skill > 0)
            {
                switch (skill)
                {
                    ...
                    case 90: return CheckForProperty(item, (int)eProperty.Skill_Long_bows) || 
                                    ...
                                    CheckForProperty(item, (int)eProperty.Skill_RecurveBow); //  archery?  
                    ...
                    case 300: return CheckForProperty(item, (int)eProperty.Skill_Slashing) ||
                                        ...
                                        CheckForProperty(item, (int)eProperty.Skill_Composite) ||
                                        CheckForProperty(item, (int)eProperty.Skill_RecurveBow) ||
