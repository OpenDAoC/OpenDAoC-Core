...
namespace DOL.GS
{
	...
	public class NpcTemplate : INpcTemplate
	{
		...
		public NpcTemplate(DbNpcTemplates data)
		{
			if (data == null)
		...
		{
			var objs = GameServer.Database.SelectAllObjects<DbNpcTemplates>();
			int free_id = 1;
			int doubleidcheck = 0;
			foreach (DbNpcTemplates dbtemplate in objs.OrderBy(x => x.TemplateId))
			{
		...
		{
			DbNpcTemplates tmp = GameServer.Database.FindObjectByKey<DbNpcTemplates>(TemplateId);
			bool add = false;

			if (tmp == null)
			{
				tmp = new DbNpcTemplates();
				add = true;
			}
