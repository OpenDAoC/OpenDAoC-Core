	...
	public class PlayerTradeWindow : ITradeWindow
	{
		...
		public bool Repairing
		{
			...
			{ 		
				...
				DbInventoryItems itemToRepair = (DbInventoryItems) m_partnerWindow.TradeItems[0];
		...
		public bool Combine
		{
			...
			{ 
				...
				DbInventoryItems itemToCombine = (DbInventoryItems) m_partnerWindow.TradeItems[0];
		...
		public bool AcceptTrade()
		{
			lock (Sync)
			{
				...
				if(m_combine == true)
				{
                    ...
                    {
                        foreach (DbInventoryItems i in (ArrayList)crafter.TradeWindow.TradeItems.Clone())
                        {
                            if (i.Object_Type == (int)eObjectType.AlchemyTincture)
                            {
                                if (m_owner.GetCraftingSkillValue(eCraftingSkill.Alchemy) > 0)
				...
				{
					GamePlayer crafter = (m_recipiant == true ? m_owner : partner);
					DbInventoryItems itemToRepair = (DbInventoryItems)(m_recipiant == true ? m_partnerWindow.TradeItems[0] : TradeItems[0]);
					if(itemToRepair != null)
					{
				...
				else
				{
                    ...
                    foreach (DbInventoryItems item in ownerTradeItems)
                    {
                    ...
                    foreach (DbInventoryItems item in partnerTradeItems)
                    {
					...
					foreach(DbInventoryItems item in ownerTradeItems)
					{
						...
						DbInventoryItems itemtoadd = item;
