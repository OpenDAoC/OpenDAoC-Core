...
namespace DOL.GS.Commands
{
	...
	public class ItemCommandHandler : AbstractCommandHandler, ICommandHandler
	{
					...
					case "blank":
						{
                            ...
                            {
                                Name = "(blank item)",
                                Id_nb = DbInventoryItems.BLANK_ITEM
                            };
					...
					case "create":
						{
							...
							else
							{
								...
								DbInventoryItems item = GameInventoryItem.Create(template);
								if (item.IsStackable)
								{
									item.Count = count;
								}
					...
					case "extension":
						{
							...
							if (item.Template is DbItemUniques || (item.Template is DbItemTemplates && (item.Template as DbItemTemplates).AllowUpdate))
							{
					...
					case "saveunique":
						{
							...
							{
								DbItemUniques itemUnique = item.Template as DbItemUniques;
					...
					case "addunique":
						{
							...
							if (idnb == string.Empty && (item.AllowAdd == false || item.Id_nb == DbInventoryItems.BLANK_ITEM || args[1].ToLower() == "addunique"))
							{
							...
							{
								if (args[1].ToLower() == "save" && item.Template is DbItemUniques)
								{
		...
		private void UpdateAllowed(DbInventoryItems item, GameClient client)
		{
			if (item.Template is DbItemUniques)
			{
