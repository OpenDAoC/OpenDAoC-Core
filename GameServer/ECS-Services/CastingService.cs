        ...
        private static void TickInternal(CastingComponent castingComponent)
        {
            try
            {
                ...
                if (monitor.IsLongTick(out long elapsedMs) && log.IsWarnEnabled)
                    log.Warn($"Long {Instance.ServiceName}.{nameof(Tick)} for {castingComponent.Owner.Name}({castingComponent.Owner.ObjectID}) Spell: {castingComponent.SpellHandler?.Spell?.Name} Time: {elapsedMs}ms");
            }
            catch (Exception e)
            {
