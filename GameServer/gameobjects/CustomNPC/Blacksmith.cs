...
public class Blacksmith : GameNPC
{
    ...
    protected void BlacksmithDialogResponse(GamePlayer player, byte response)
    {
        ...
        var item = (DbInventoryItems) itemWeak.Target;
    ...
    private bool CanBeRepaired(DbInventoryItems item)
    {
        if (item == null || item.SlotPosition == (int) eInventorySlot.Ground
    ...
    private long CalculateCost(DbInventoryItems item)
    {
        long NeededMoney = 0;
    ...
    private void Repair(DbInventoryItems item, GamePlayer player)
    {
        var ToRecoverCond = item.MaxCondition - item.Condition;
