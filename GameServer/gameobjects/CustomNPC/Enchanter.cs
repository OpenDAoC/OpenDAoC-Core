			...
			if (item.Level >= 10 && item.IsCrafted)
			{
				if (item.Object_Type != (int) eObjectType.Magical && item.Object_Type != (int) eObjectType.Bolt && item.Object_Type != (int) eObjectType.Poison)
				{
					if (item.Bonus == 0)
					{
                        ...
                        player.Client.Out.SendCustomDialog(LanguageMgr.GetTranslation(player.Client, "GameNPC.Enchanter.ReceiveItem.Cost", WalletHelper.ToString(CalculEnchantPrice(item))), new CustomDialogResponse(EnchanterDialogResponse));
                    }
					else
		...
		protected void EnchanterDialogResponse(GamePlayer player, byte response)
		{
			...
			if (player.Wallet.GetMoney() < Fee)
			{
				...
				ChatUtil.SendSayMessage(player, "GameNPC.Enchanter.Response.NotEnoughMoney", 
					GetName(0, true), 
					WalletHelper.ToString(Fee), 
					item.GetName(0, false));
                //SayTo(player, eChatLoc.CL_SystemWindow, LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Enchanter.Response.NotEnoughMoney", WalletHelper.GetString(Fee)));
                return;
			}
			if (item.Level < 50)
            ...
            //player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "GameNPC.Enchanter.Response.YouGive", 
                                    //GetName(0, false, player.Client.Account.Language, this), WalletHelper.GetString(Fee)), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            player.Wallet.RemoveMoney(Fee, null);
