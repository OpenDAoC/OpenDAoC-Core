	...
	public class GameStableMaster : GameMerchant
	{
		...
		public override void OnPlayerBuy(GamePlayer player, int item_slot, int number)
		{
			...
			if (item.Name.ToUpper().Contains("TICKET TO") || item.Description.ToUpper() == "TICKET")
			{
				...
				DbInventoryItems ticket = player.Inventory.GetFirstItemByName(item.Name, eInventorySlot.FirstBackpack, eInventorySlot.LastBackpack);
				if (ticket != null)
		...
		private bool isItemInMerchantList(DbInventoryItems item)
		{
