...
namespace DOL.GS
{
	...
	public abstract class GameObject : Point3D
	{
                ...
                default:
                    {
                        if (obj is GameNPC)
                        {
                            var translation = (DbLanguageGameNpcs)LanguageMgr.GetTranslation(lang, obj);
		...
		public virtual bool ReceiveItem(GameLiving source, DbInventoryItems item)
		{
		...
		{
			DbInventoryItems item = player.Inventory.GetFirstItemByID(str, eInventorySlot.Min_Inv, eInventorySlot.Max_Inv);
			if (item != null)
				return true;
			return false;
		}
