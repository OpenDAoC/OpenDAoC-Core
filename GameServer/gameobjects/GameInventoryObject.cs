...
namespace DOL.GS
{
	...
	public interface IGameInventoryObject
	{
		...
		IList<DbInventoryItems> DBItems(GamePlayer player = null);
		Dictionary<int, DbInventoryItems> GetClientInventory(GamePlayer player);
		...
		bool OnAddItem(GamePlayer player, DbInventoryItems item);
		bool OnRemoveItem(GamePlayer player, DbInventoryItems item);
	...
	public static class GameInventoryObjectExtensions
	{
		...
		/// </summary>
		public static Dictionary<int, DbInventoryItems> GetClientItems(this IGameInventoryObject thisObject, GamePlayer player)
		{
			var inventory = new Dictionary<int, DbInventoryItems>();
			int slotOffset = thisObject.FirstClientSlot - thisObject.FirstDBSlot;
			foreach (DbInventoryItems item in thisObject.DBItems(player))
			{
				if (item != null)
				{
		...
		/// </summary>
		public static IDictionary<int, DbInventoryItems> MoveItemFromObject(this IGameInventoryObject thisObject, GamePlayer player, eInventorySlot fromClientSlot, eInventorySlot toClientSlot)
		{
			...
			{
				Dictionary<int, DbInventoryItems> inventory = thisObject.GetClientInventory(player);
				...
				DbInventoryItems fromItem = inventory[(int)fromClientSlot];
				DbInventoryItems toItem = player.Inventory.GetItem(toClientSlot);
				...
				DbInventoryItems objectItem = GameInventoryItem.Create(fromItem);

				player.Inventory.AddTradeItem(toClientSlot, objectItem);

				var updateItems = new Dictionary<int, DbInventoryItems>(1);
		...
		/// </summary>
		public static IDictionary<int, DbInventoryItems> MoveItemToObject(this IGameInventoryObject thisObject, GamePlayer player, eInventorySlot fromClientSlot, eInventorySlot toClientSlot)
		{
			...
			DbInventoryItems fromItem = player.Inventory.GetItem(fromClientSlot);
			...
			lock (thisObject.LockObject())
			{
				...
				{
					DbInventoryItems toItem = inventory[(int)toClientSlot];
		...
		/// </summary>
		public static IDictionary<int, DbInventoryItems> MoveItemInsideObject(this IGameInventoryObject thisObject, GamePlayer player, eInventorySlot fromSlot, eInventorySlot toSlot)
		{
			lock (thisObject.LockObject())
			{
				IDictionary<int, DbInventoryItems> inventory = thisObject.GetClientInventory(player);

				if (!inventory.ContainsKey((int)fromSlot))
					return null;

				var updateItems = new Dictionary<int, DbInventoryItems>(2);
				DbInventoryItems fromItem = null, toItem = null;
