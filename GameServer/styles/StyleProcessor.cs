...
namespace DOL.GS.Styles
{
	...
	public class StyleProcessor
	{
		...
		public static bool CanUseStyle(AttackData lastAttackData, GameLiving living, Style style, DbInventoryItems weapon)
		{
		...
		public static void TryToUseStyle(GameLiving living, Style style)
		{
			...
			lock (player)
			{
				...
				DbInventoryItems weapon = (style.WeaponTypeRequirement == (int) eObjectType.Shield) ? player.Inventory.GetItem(eInventorySlot.LeftHandWeapon) : player.ActiveWeapon;
		...
		public static bool ExecuteStyle(GameLiving living, GameLiving target, Style style, DbInventoryItems weapon, double unstyledDamage, double unstyledDamageCap, eArmorSlot armorHitLocation, IList<ISpellHandler> styleEffects, out int styleDamage, out int animationId)
		{
			styleDamage = 0;
			animationId = 0;
			...
			lock (living)
			{
				...
				else
				{
					...
					if (staticGrowth)
					{
						...
						DbInventoryItems armor = target.Inventory?.GetItem((eInventorySlot) armorHitLocation);
		...
		protected static bool CheckWeaponType(Style style, GameLiving living, DbInventoryItems weapon)
		{
			...
			switch (style.WeaponTypeRequirement)
			{
				case Style.SpecialWeaponType.DualWield:
					...
					DbInventoryItems rightHand = player.ActiveWeapon;
