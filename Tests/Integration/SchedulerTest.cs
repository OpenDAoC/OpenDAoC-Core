...
public class SchedulerTest
{
	...
	public void Scheduler_TimerStartLatency_LowerThan()
	{
		...
		var task = scheduler.Start(() =>
		{
			finished = SimpleScheduler.Ticks;
			return 0;
		}, 1);
	...
	public void Scheduler_MultiInsertInterval_FixedCountRun()
	{
		...
		var longTasks = Enumerable.Range(0, 10).Select(i => scheduler.Start(() =>
		{
			longCount[i]++;
			return longCount[i] == 5 ? 0 : 500;
		}, 500)).ToArray();
		...
		var shortTasks = Enumerable.Range(0, 10).Select(i => scheduler.Start(() =>
		{
			shortCount[i]++;
			return shortCount[i] == 10 ? 0 : 100;
		}, 1)).ToArray();
