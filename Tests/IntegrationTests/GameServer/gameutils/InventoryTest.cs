...
namespace DOL.Tests.Integration.Server
{
	...
	public class InventoryTest : ServerTests
	{
		...
		public void InventoryFromIT()
		{
			...
			DbInventoryItems ii = GameInventoryItem.Create(itemt);
			...
			var iicheck = GameServer.Database.FindObjectByKey<DbInventoryItems>(ii.ObjectId);
			Assert.IsNotNull(iicheck, "ii-t #2 : saved in db " + ii.Template.Id_nb + " to " + ii.OwnerID);
			GameServer.Database.DeleteObject(ii);
			iicheck = GameServer.Database.FindObjectByKey<DbInventoryItems>(ii.ObjectId);
		...
		public void InventoryFromIU()
		{
			...
			DbInventoryItems ii = GameInventoryItem.Create(itemu);
			player.Inventory.AddItem(eInventorySlot.FirstBackpack, ii);
			...
			var iucheck = GameServer.Database.FindObjectByKey<DbItemUniques>(itemu.Id_nb);
			...
			iucheck = GameServer.Database.FindObjectByKey<DbItemUniques>(itemu.Id_nb);
		...
		public void InventoryFromNull()
		{
			...
			DbInventoryItems ii = new DbInventoryItems();
			player.Inventory.AddItem(eInventorySlot.FirstBackpack, ii);
