...
namespace DOL.Tests.Integration.Database
{
	...
	public class InterfaceTests
	{
		...
		public void TestSingleAdd()
		{
			...
			ClassicAssert.IsFalse(obj.IsPersisted, "Test Object should not have Persisted Flag before adding...");
			ClassicAssert.IsTrue(obj.Dirty, "Test Object should be Dirty before adding...");
			
			var inserted = Database.AddObject(obj);
			
			ClassicAssert.IsTrue(inserted, "Test Object should be inserted successfully, something went wrong...");
			ClassicAssert.IsTrue(obj.IsPersisted, "Test Object should have Persisted Flag after adding to database...");
			ClassicAssert.IsFalse(obj.Dirty, "Test Object should not be Dirty after adding to database...");
			ClassicAssert.AreEqual(objId, obj.ObjectId, "Test Object should have kept its ObjectId from Creation...");
			
			var retrieved = Database.FindObjectByKey<TestTable>(obj.ObjectId);
			
			ClassicAssert.IsNotNull(retrieved, "Test Object previously added should be retrieved from database...");
			ClassicAssert.AreEqual(objId, retrieved.ObjectId, "Test Object previously added and retrieved object should have same ObjectId...");
			ClassicAssert.AreEqual(obj.TestField, retrieved.TestField, "Test Object previously added and retrieved object should have equals fields...");
		}
		...
		public void TestMultipleAddSameTable()
		{
			...
			{
				ClassicAssert.IsFalse(obj.IsPersisted, "Test Objects should not have Persisted Flag before adding...");
				ClassicAssert.IsTrue(obj.Dirty, "Test Objects should be Dirty before adding...");
			}
			
			var inserted = Database.AddObject(objs);
			ClassicAssert.IsTrue(inserted, "Test Objects should be inserted successfully, something went wrong...");
			foreach (var obj in objs)
			{
				ClassicAssert.IsTrue(obj.IsPersisted, "Test Objects should have Persisted Flag after adding to database...");
				ClassicAssert.IsFalse(obj.Dirty, "Test Objects should not be Dirty after adding to database...");
			}
			...
			ClassicAssert.IsNotNull(retrieved, "Test Objects retrieved Collection should not be null...");
		...
		public void TestMultipleAddDifferentTable()
		{
			...
			foreach (var obj in objs)
			{
				ClassicAssert.IsFalse(obj.IsPersisted, "Different Test Objects should not have Persisted Flag before adding...");
				ClassicAssert.IsTrue(obj.Dirty, "Different Test Objects should be Dirty before adding...");
			}
			
			var inserted = Database.AddObject(objs);
			
			ClassicAssert.IsTrue(inserted, "Different Test Objects should be inserted successfully, something went wrong...");
			foreach (var obj in objs)
			{
				ClassicAssert.IsTrue(obj.IsPersisted, "Test Objects should have Persisted Flag after adding to database...");
				ClassicAssert.IsFalse(obj.Dirty, "Test Objects should not be Dirty after adding to database...");
			...
			foreach (var obj in objsTestTableRelations)
			{
				...
				{
					ClassicAssert.IsTrue(ent.IsPersisted, "TestTableRelationEntries Objects should have Persisted Flag after adding to database...");
					ClassicAssert.IsFalse(ent.Dirty, "TestTableRelationEntries Objects should not be Dirty after adding to database...");
				}
			}
			...
			ClassicAssert.IsNotNull(retrievedTestTable, "TestTable Objects retrieved Collection should not be null...");
			...
			ClassicAssert.IsNotNull(retrievedTestTableAutoInc, "TestTableAutoInc Objects retrieved Collection should not be null...");
			...
			ClassicAssert.IsNotNull(retrievedTestTableRelations, "TestTableRelations Objects retrieved Collection should not be null...");
		...
		{
			DataObject dbo = null;
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.AddObject(dbo), "Adding a Single Null Object Should throw NullReferenceException...");
		}
		...
		public void TestSingleAddNonRegistered()
		{
			...
			ClassicAssert.IsFalse(added, "Adding a Single non registered Object should not return success...");
			ClassicAssert.IsFalse(dbo.IsPersisted, "Adding a Single non registered Object should not have Persisted Flag...");
		}
		...
		public void TestMultipleAddWithNull()
		{
			...
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.AddObject(dbo), "Adding a Multiple with Null Objects Should throw NullReferenceException...");
		}
		...
		public void TestMultipleAddNonRegistered()
		{
			...
			var added = Database.AddObject(dbo);
			ClassicAssert.IsFalse(added, "Adding Multiple non registered Object should not return success...");
			ClassicAssert.IsFalse(dbo[1].IsPersisted, "Adding a Multiple non registered Object should not have Persisted Flag on unregistered Object...");
			ClassicAssert.IsTrue(dbo[0].IsPersisted, "Adding a Multiple non registered Object should have Persisted Flag on registered Object...");
		}
		...
		public void BenchSingleAdd()
		{
			...
			ClassicAssert.IsEmpty(Database.SelectAllObjects<TestTable>(), "Database shouldn't have any record For TestTable.");
			ClassicAssert.IsEmpty(Database.SelectAllObjects<TestTableRelations>(), "Database shouldn't have any record For TestTable.");
			ClassicAssert.IsEmpty(Database.SelectAllObjects<TestTableRelationsEntries>(), "Database shouldn't have any record For TestTable.");
		...
		public void TestSingleSave()
		{
			...
			ClassicAssert.IsTrue(obj.Dirty, "Changing TestTable Object should set Dirty Flag...");
			
			var saved = Database.SaveObject(obj);
			
			ClassicAssert.IsTrue(saved, "Changed Object Should be Saved Successfully in database...");
			ClassicAssert.IsFalse(obj.Dirty, "Changed Object should not have Dirty flag after Saving...");
			
			var retrieve = Database.FindObjectByKey<TestTable>(obj.ObjectId);
			
			ClassicAssert.IsNotNull(retrieve, "Changed Object Should be retrieved from database...");
			ClassicAssert.AreEqual(obj.TestField, retrieve.TestField, "Previously Changed Object and newly retrieved Object should have the same field value...");
		}
		...
		public void TestMultipleSaveSameTable()
		{
			...
			foreach (var obj in objs)
			{
				...
				current++;
				ClassicAssert.IsTrue(obj.Dirty, "Changing TestTable Objects should set Dirty Flag...");
			}
			
			var saved = Database.SaveObject(objs);
			
			ClassicAssert.IsTrue(saved, "Changed Objects Should be Saved Successfully in database...");
			foreach (var obj in objs)
				ClassicAssert.IsFalse(obj.Dirty, "Changed Objects should not have Dirty flag after Saving...");
			
			var retrieved = Database.FindObjectsByKey<TestTable>(objs.Select(obj => obj.ObjectId));
			
			ClassicAssert.IsNotNull(retrieved, "Changed Objects Collection Should be retrieved from database...");
		...
		public void TestMultipleSaveDifferentTable()
		{
			...
			{
				foreach (var ent in obj.Entries)
					ClassicAssert.IsFalse(ent.Dirty, "Changed Objects Relations should not have Dirty flag after Saving...");
			}
		...
		{
			DataObject obj = null;
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.SaveObject(obj), "Trying to save a null object should throw NullReferenceException...");
		}
		...
		public void TestSingleSaveNonRegistered()
		{
			...
			obj.Dirty = true;
			var saved = Database.SaveObject(obj);
			ClassicAssert.IsFalse(saved, "Trying to save a non registered object should not return success...");
			ClassicAssert.IsTrue(obj.Dirty, "Failing to save a non registered object should not remove Dirty Flag...");
		}
		...
		public void TestMultipleSaveNull()
		{
			...
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.SaveObject(dbo), "Saving a Collection with Null Objects Should throw NullReferenceException...");
		}
		...
		public void TestMultipleSaveNonRegistered()
		{
			...
			ClassicAssert.IsFalse(saved, "Saving Multiple non registered Object should not return success...");
			ClassicAssert.IsFalse(dbo[0].Dirty, "Saving a Collection with non registered Object should not have Dirty Flag on registered Object...");
			ClassicAssert.IsTrue(dbo[1].Dirty, "Saving a Collection with non registered Object should have Dirty Flag on non-registered Object...");
		}
		#endregion
		...
		public void TestSingleDelete()
		{
			...
			ClassicAssert.IsTrue(obj.IsPersisted, "Added Object should have Persisted Flag...");
			
			var deleted = Database.DeleteObject(obj);
			
			ClassicAssert.IsTrue(deleted, "Added Object should be Successfully Deleted from database...");
			ClassicAssert.IsFalse(obj.IsPersisted, "Deleted Object should not have Persisted Flag");
			ClassicAssert.IsTrue(obj.IsDeleted, "Deleted Object should have Deleted Flag");
			
			var retrieve = Database.FindObjectByKey<TestTable>(obj.ObjectId);
			ClassicAssert.IsNull(retrieve, "Retrieving Deleted Object should return null...");
		}
		...
		public void TestMultipleDeleteSameTable()
		{
			...
			foreach (var obj in objs)
				ClassicAssert.IsTrue(obj.IsPersisted, "Added Objects should have Persisted Flag...");
			
			var deleted = Database.DeleteObject(objs);
			
			ClassicAssert.IsTrue(deleted, "Added Objects should be Successfully Deleted from database...");
			foreach (var obj in objs)
			{
				ClassicAssert.IsFalse(obj.IsPersisted, "Deleted Objects should not have Persisted Flag");
				ClassicAssert.IsTrue(obj.IsDeleted, "Deleted Objects should have Deleted Flag");
			}
			
			var retrieve = Database.FindObjectsByKey<TestTable>(objs.Select(obj => obj.ObjectId));
			ClassicAssert.IsNotNull(retrieve, "Retrieving Deleted Objects Collection should not return null...");
			ClassicAssert.IsEmpty(retrieve.Where(obj => obj != null), "Retrieved Deleted Objects Collection should be empty...");
		}
		...
		public void TestMultipleDeleteDifferentTable()
		{
			...
			ClassicAssert.IsTrue(inserted, "Object Should be inserted to test Delete Cases...");
			
			// Delete Objects
			var deleted = Database.DeleteObject(objs);
			
			ClassicAssert.IsTrue(deleted, "Object should be deleted successfully...");
			...
			foreach (var obj in objsTestTableRelations)
			{
				foreach (var ent in obj.Entries)
				{
					ClassicAssert.IsTrue(ent.IsDeleted, "Deleted Objects should have Deleted Flag...");
					ClassicAssert.IsFalse(ent.IsPersisted, "Deleted Objects should not have Persisted Flag...");
				}
			}
			...
			ClassicAssert.IsNotNull(retrieved, "Deleted Objects Collection Should be retrieved Empty not null from database...");
			ClassicAssert.IsEmpty(retrieved.Where(obj => obj != null), "Deleted Objects Collection Should be retrieved Empty from database...");
			...
			ClassicAssert.IsNotNull(relRetrieved, "Deleted Objects Collection Should be retrieved Empty not null from database...");
			ClassicAssert.IsEmpty(relRetrieved.Where(obj => obj != null), "Deleted Objects Collection Should be retrieved Empty from database...");
		}
		...
		{
			DataObject obj = null;
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.DeleteObject(obj), "Trying to delete a null object should throw NullReferenceException...");
		}
		...
		public void TestSingleDeleteNonRegistered()
		{
			DataObject obj = new TableNotRegistered();
			var deleted = Database.DeleteObject(obj);
			ClassicAssert.IsFalse(deleted, "Trying to Delete a non registered object should not return success...");
			ClassicAssert.IsFalse(obj.IsDeleted, "Failing to Delete a non registered object should not set Deleted Flag...");
		}
		...
		public void TestMultipleDeleteNull()
		{
			...
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.DeleteObject(dbo), "Deleting a Collection with Null Objects Should throw NullReferenceException...");
		}
		...
		public void TestMutipleDeleteNonRegistered()
		{
			...
			ClassicAssert.IsFalse(deleted, "Deleting Multiple non registered Object should not return success...");
			ClassicAssert.IsTrue(dbo[0].IsDeleted, "Deleting a Collection with non registered Object should set Deleted Flag on registered Object...");
			ClassicAssert.IsFalse(dbo[1].IsDeleted, "Deleting a Collection with non registered Object should not set Deleted Flag non-registered Object...");
		}
		...
		public void TestSingleFindObjectByKey()
		{
			...
			ClassicAssert.IsTrue(inserted, "Find Object By Key Test Could not add object in database...");
			
			var retrieve = Database.FindObjectByKey<TestTable>(obj.ObjectId);
			
			ClassicAssert.IsNotNull(retrieve, "Find Object By Key Could not retrieve previously added Object...");
			ClassicAssert.AreEqual(obj.ObjectId, retrieve.ObjectId, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieve.TestField, "Find Object By Key Should return similar Object to created one...");
			
			var retrieveCase = Database.FindObjectByKey<TestTable>(obj.ObjectId.ToUpper());
			
			ClassicAssert.IsNotNull(retrieveCase, "Find Object By Key Could not retrieve previously added Object using Case Mismatch...");
			ClassicAssert.AreEqual(obj.ObjectId, retrieveCase.ObjectId, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieveCase.TestField, "Find Object By Key Should return similar Object to created one...");
		}
		...
		public void TestMultipleFindObjectByKey()
		{
			...
			ClassicAssert.IsTrue(inserted, "Find Object By Key Test Could not add objects in database...");
			
			var retrieve = Database.FindObjectsByKey<TestTable>(objs.Select(obj => obj.ObjectId));
			
			ClassicAssert.IsNotNull(retrieve, "Find Object By Key Could should not return a null collection...");
			ClassicAssert.IsNotEmpty(retrieve.Where(obj => obj != null), "Find Object By Key Could not retrieve previously added Objects...");
			...
			ClassicAssert.IsNotNull(retrieveCase, "Find Object By Key Could should not return a null collection...");
			ClassicAssert.IsNotEmpty(retrieveCase.Where(obj => obj != null), "Find Object By Key Could not retrieve previously added Objects using Case Mismatch...");
		...
		public void TestSingleFindObjectByKeyPrimaryKey()
		{
			...
			ClassicAssert.IsNotNull(retrieve, "Find Object By Key Could not retrieve previously added Object...");
			ClassicAssert.AreEqual(obj.PrimaryKey, retrieve.PrimaryKey, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieve.TestField, "Find Object By Key Should return similar Object to created one...");
			
			var retrieveCase = Database.FindObjectByKey<TestTablePrimaryKey>(obj.PrimaryKey.ToUpper());
			
			ClassicAssert.IsNotNull(retrieveCase, "Find Object By Key Could not retrieve previously added Object using Case Mismatch...");
			ClassicAssert.AreEqual(obj.PrimaryKey, retrieveCase.PrimaryKey, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieveCase.TestField, "Find Object By Key Should return similar Object to created one...");
		}
		...
		public void TestSingleFindObjectByKeyPrimaryKeyAutoInc()
		{
			...
			ClassicAssert.IsNotNull(retrieveCast, "Find Object By Key Could not retrieve previously added Object using Numeric Cast...");
			ClassicAssert.AreEqual(obj.PrimaryKey, retrieveCast.PrimaryKey, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieveCast.TestField, "Find Object By Key Should return similar Object to created one...");
			
			var retrieveString = Database.FindObjectByKey<TestTableAutoInc>(obj.PrimaryKey.ToString());
			
			ClassicAssert.IsNotNull(retrieveString, "Find Object By Key Could not retrieve previously added Object using String Cast...");
			ClassicAssert.AreEqual(obj.PrimaryKey, retrieveString.PrimaryKey, "Find Object By Key Should return similar Object to created one...");
			ClassicAssert.AreEqual(obj.TestField, retrieveString.TestField, "Find Object By Key Should return similar Object to created one...");
		}
		...
		public void TestMultipleFindObjectByKeyPrimaryKeyAutoInc()
		{
			...
			ClassicAssert.IsNotNull(retrieveCast, "Find Object By Key Could should not return a null collection...");
			ClassicAssert.IsNotEmpty(retrieveCast.Where(obj => obj != null), "Find Object By Key Could not retrieve previously added Objects using Numeric Cast...");
			...
			ClassicAssert.IsNotNull(retrieveString, "Find Object By Key Could should not return a null collection...");
			ClassicAssert.IsNotEmpty(retrieveString.Where(obj => obj != null), "Find Object By Key Could not retrieve previously added Objects using String Cast...");
		...
		public void TestSingleFindObjectByKeyWithNull()
		{
			...
			ClassicAssert.IsNull(dbo, "Searching an Object By Key with a Null Key should return a null object...");
		}
		...
		public void TestMultipleFindObjectByKeyWithNull()
		{
			...
			ClassicAssert.IsNotEmpty(result, "Searching with multiple keys including null shoud not return an empty collection...");
			ClassicAssert.IsNull(result[0], "Searching with multiple keys including null should return null data object for null key...");
			ClassicAssert.IsNotNull(result[1], "Searching with multiple keys including null should return a valid data object for non null key...");
		}
		...
		{
			ClassicAssert.Throws(typeof(DatabaseException), () => Database.FindObjectByKey<TableNotRegistered>(1), "");
		}
		...
		{
			ClassicAssert.Throws(typeof(DatabaseException), () => Database.FindObjectByKey<TableNotRegistered>(new object[] { 1, 2 }), "");
		}
		...
		public void TestSelectObjects()
		{
			...
			ClassicAssert.IsNotEmpty(allobjects, "Select Objects Test Need some Data to be Accurate...");
			...
			ClassicAssert.IsTrue(Database.DeleteObject(Database.SelectObjects<TestTable>(DB.Column("TestField").IsNull())));
			...
			ClassicAssert.AreEqual(1, resultsWithTestfieldNull.Count);
		}
		...
		public void TestSelectObjectsWithMultipleWhereClause()
		{
			...
			ClassicAssert.IsTrue(delete, "TestTable Objects should be deleted successfully...");
			ClassicAssert.IsEmpty(Database.SelectAllObjects<TestTable>(), "This test needs an Empty Table to Run Successfully...");
			...
			ClassicAssert.IsTrue(added, "TestTable Objects should be added successfully...");
			...
			ClassicAssert.IsNotNull(retrieve, "Retrieve Sets from Select Objects should not return null value...");
			ClassicAssert.IsNotEmpty(retrieve, "Retrieve Set from Select Objects should not be Empty...");
			
			foreach (var sets in objectByGroup)
			{
				ClassicAssert.IsNotEmpty(sets.Objects, "Retrieve SubSets from Select Objects should not be Empty...");
				ClassicAssert.IsTrue(sets.Objects.All(obj => obj.TestField.Equals(sets.Grp, StringComparison.OrdinalIgnoreCase)),
			...
			ClassicAssert.IsNotNull(retrieveMany, "Retrieve Sets from Select Objects should not return null value...");
			ClassicAssert.IsNotEmpty(retrieveMany, "Retrieve Set from Select Objects should not be Empty...");
			...
			ClassicAssert.IsNotNull(retrieveManyWithMissing, "Retrieve Sets from Select Objects should not return null value...");
			ClassicAssert.IsNotEmpty(retrieveManyWithMissing, "Retrieve Set from Select Objects should not be Empty...");
		...
		public void TestSelectObjectsWithNull()
		{
			...
			ClassicAssert.IsNotEmpty(allobjects, "This Test Need some Data to be Accurate...");

			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.SelectObject<TestTable>((WhereClause)null), "");
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.SelectObjects<TestTable>((WhereClause)null), "");
			ClassicAssert.Throws(typeof(ArgumentNullException), () => Database.MultipleSelectObjects<TestTable>(null));
		}
		...
		public void TestSelectAll()
		{
			...
			ClassicAssert.IsNotNull(objs, "Retrieving from a Registered Table should not return null...");
			...
			ClassicAssert.IsNotNull(objs, "TestTable Select All from an Empty table should not return null...");
			ClassicAssert.IsEmpty(objs, "TestTable Select All from an Empty table should return an Empty Collection...");
			
			objs = Enumerable.Range(0, 10).Select(i => new TestTable { TestField = string.Format("Test Select All Object #{0}", i) }).ToList();
			
			var inserted = Database.AddObject(objs);
			
			ClassicAssert.IsTrue(inserted, "TestTable Objects should be inserted successfully...");
			
			var retrieve = Database.SelectAllObjects<TestTable>();
			ClassicAssert.IsNotEmpty(retrieve, "TestTable Objects Select All should return previously inserted objects...");
		...
		{
			ClassicAssert.Throws(typeof(DatabaseException), () => Database.SelectAllObjects<TableNotRegistered>(), "Trying to Query a Non Registered Table should throw a DatabaseException...");
		}
		#endregion
		...
		public void TestCountObject()
		{
			...
			ClassicAssert.AreEqual(0, count, "Test Table shouldn't Have any Object after deleting all records...");
			...
			ClassicAssert.AreEqual(10, newCount, "Test Table should return same object count as added collection...");
			
			var whereCount = Database.GetObjectCount<TestTable>("1");
			
			ClassicAssert.AreEqual(10, whereCount, "Test Table should return same object count as added collection...");
			
			var filterCount = Database.GetObjectCount<TestTable>("`TestField` LIKE '%1'");
			
			ClassicAssert.AreEqual(1, filterCount, "Test Table should return same object count as filtered collection...");
		}
		...
		{
			ClassicAssert.Throws(typeof(DatabaseException), () => Database.GetObjectCount<TableNotRegistered>(), "Get Object Count should throw exception for unregistered tables...");
			ClassicAssert.Throws(typeof(DatabaseException), () => Database.GetObjectCount<TableNotRegistered>("1"), "Get Object Count should throw exception for unregistered tables...");
		}
		#endregion
		...
		public virtual void TestEscape()
		{
			...
			ClassicAssert.AreEqual("''", Database.Escape(test), "Sqlite String Escape Test Failure...");
		}
		...
		{
			ClassicAssert.Throws(typeof(NullReferenceException), () => Database.Escape(null), "SQL Escape string with Null value should throw Null Reference Exception...");
		}
		#endregion
	}
}
